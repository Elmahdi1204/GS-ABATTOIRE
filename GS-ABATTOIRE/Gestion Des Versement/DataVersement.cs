...
namespace GS_ABATTOIRE.Gestion_Des_Versement
{
    internal class DataVersement
    {
        ...
        public static void List_des_versement_achats(BunifuDataGridView bunifuDataGridView, String txt)
        {
            ...
            {
                Connexion.conn.Open();
                SqlCommand sql = new SqlCommand("select idversement , nomkottas , nomfournisseur , montant , Versement.date from Versement , Kottas   , Fournisseurs where Versement.type='Achats' and  Kottas.idfournisseur =Fournisseurs.idfournisseur  and Kottas.idkottas = Versement.idversement and nomfournisseur LIKE N'%"+txt+ "%' order by Versement.date desc ;", Connexion.conn);
